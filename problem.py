...
class LP:
    def __init__(self, G:List[List[float]], client_locations: List[List[int]], k: int):
        self.G = G
    ...
    def get_variable_solution(self):
        if self.status == pywraplp.Solver.OPTIMAL:
            return [self.X[ind].solution_value() for ind in range(len(self.X))], [self.Y[address].solution_value() for address in self.Y.keys()]
        else:
            print("Not optimal")
            return [], []
